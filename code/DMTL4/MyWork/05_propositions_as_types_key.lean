...
/-!
## #2: Required Learning: Propositions as Types in Lean
...
inductive Product (α β : Type) : Type
| intro (a : α) (b : β)

def swap : Product Bool Nat → Product Nat Bool
| Product.intro a b => Product.intro b a
...
where you'll distinguish the two different cases. Use _ for any
arguments that are not used in computing the resulting proof
values.
-/
...
Finally, construct two proofs of KIFC ∨ CIFC. Call them pfL
and pfR, then apply the "or_commutes" theorem (function) to
the required three argument values---a first proposition P,
a second proposition, Q, and a proof of P ∨ Q, to derive a
corresponding proof of Q ∨ P, and see the #check commands
to confirm that the returned proof values are proofs of the
proposition, CIFC ∨ KIFC.
-/

def pfL : KIFC ∨ CIFC := Or.inl pfKIFC1
def pfR : KIFC ∨ CIFC := Or.inr pfCIFC

-- Both expressions should return a proof of CIFC ∨ KIFC
#check or_commutes KIFC CIFC pfL
#check or_commutes KIFC CIFC pfR
