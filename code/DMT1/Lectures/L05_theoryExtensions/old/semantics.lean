...
open DMT1.Lectures.theoryExtensions.syntax
open DMT1.Lectures.propLogic.semantics.domain
...
| BinOp.and => Bool.and
| BinOp.or => Bool.or
| BinOp.imp => propLogic.semantics.domain.imp   -- DMT1.lecture.propLogic.semantics.domain.imp
| BinOp.iff => propLogic.semantics.domain.iff  -- likewise
...
abbrev BoolInterp := Var → Bool
abbrev NatInterp := Nat → Bool
...
def eval : Expr → BoolInterp → NatInterp → Bool
| lit_expr b,             bi, ni  => b
| (var_expr v),           bi, ni  => bi v
| (un_op_expr op e),      bi, ni => (evalUnOp op) (eval e bi)
| (bin_op_expr op e1 e2), bi, ni  => (evalBinOp op) (eval e1 i) (eval e2 bi)
| arithPredExpr  => _
| arithOpExpr aoe => _
